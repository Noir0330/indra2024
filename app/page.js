...
export default async function Home({ searchParams }) {
  const searchData = await getBlogs(searchParams);
  // console.log("data in home page => ", data);

  const { blogs, currentPage, totalPages } = searchData;
  ...
  return (
    <div>
      <div className="display-flex">
        <p className="lead text-primary text-center">Contents</p>
        <Link
          href={`localhost:3000/phone`}
